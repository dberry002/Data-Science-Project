...
Initializing GroupKFold, preprocessor, and pipeline
```{python}
...
lasso_pipe = make_pipeline(ElasticNetCV(alphas=np.logspace(-5, 5, 100), l1_ratio=1, max_iter=20000))
```
...
numeric_features = ['M', 'W', 'D', 'L', 'GF', 'GA', 'Dif', 'Points', 'GPM', 'GAPM',
...
Finding the highest average value for each feature
```{python}
mean_coefs = coef_df.abs().mean().sort_values(ascending=False)
mean_coefs.head(10)  
```
Finding the features that were used the most often
```{python}
